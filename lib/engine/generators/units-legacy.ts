...
export function legacyUnits(skillId:string,level:Level,seed:string):Exercise{
 ...
 return {skillId,level,seed,unitScale:level<4?{units:level===1?['km','hm','dam','m','dm','cm','mm']:level===2?(source==='L'||source==='mL'?['kL','hL','daL','L','dL','cL','mL']:['kg','hg','dag','g','dg','cg','mg']):['m²','dm²','cm²'],source,target,stepFactor:level===3?100:10}:undefined,prompt:`Convierte ${value} ${source} a ${target}.`,answer,answerFormat:'number',tolerance:1e-9,errorSignatures:signatures(answer,errors),hints:[concept,`Multiplica ${value} por ${canonical(factor)}.`,`${value} × ${canonical(factor)} = ${answer} ${target}.`]};
}
