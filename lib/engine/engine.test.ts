...
it('los despejes con paréntesis usan decimales exactos y legibles',()=>{for(let i=0;i<100;i++){const e=generate('equations','skill',3,`readable-${i}`);const rhs=e.prompt.split(' = ')[1];expect(rhs).toMatch(/^\d+(?:\.\d{1,2})?$/);}});
