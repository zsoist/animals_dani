...
vi.mock('./drive-connection',()=>({driveOAuthReady:()=>false}));
...
it('reports an unconfigured integration without pretending it is connected',async()=>{vi.stubEnv('GOOGLE_DRIVE_FOLDER_ID','');expect(await driveBank()).toEqual({configured:false,oauthReady:false});expect(mocks.fetch).not.toHaveBeenCalled();});
