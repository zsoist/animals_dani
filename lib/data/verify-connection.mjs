...
const cats=await clients.student.from('cat_unlocks').select('cats(name)');assert.equal(cats.error,null);assert.ok(cats.data.some(row=>row.cats?.name==='Milo'));console.log('Milo visible y persistido');
