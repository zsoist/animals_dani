...
export function TutorWorkspace({children}:{children:ReactNode}) {
 ...
 return <div className="tutor-workspace" data-view={view}><nav className="workspace-tabs" aria-label="Espacio del profesor">{tabs.map(tab=><a key={tab.id} href={`#${tab.id}`} aria-current={view===tab.id?'page':undefined} onClick={()=>setView(tab.id)}><Icon name={tab.icon} size={20}/>{tab.label}</a>)}</nav><div className="workspace-content">{children}</div></div>;
}
