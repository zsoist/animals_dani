...
export function PracticeLab({skills}:{skills:Skill[]}) {
  ...
  async function generate() {
    ...
    try {
      ...
      const result=await askQuestions({skillId:skillId||undefined,count,level,strategy,progressive:true,prompt:`Tema de clase: ${classTopic}. Entrena únicamente esta microhabilidad. Objetivo: ${goal.trim() || selected?.description || 'Una microhabilidad de octavo'}. Estrategia: ${strategy}. ${selected ? `Microhabilidad: ${selected.name}.` : ''} Para conceptos usa opciones; para despejes usa fórmulas simbólicas.`});
  ...
  return <section className="practice-lab">
    <div className="lab-intro"><div><h2>Crear preguntas</h2><p>1. Elige cómo empezar · 2. Revisa las preguntas · 3. Guarda</p></div><Image src="/art/numa.webp" width={180} height={180} alt="Numa, el gato tutor, con su cuaderno y sus gafas torcidas" /></div>
    ...
    </div><div hidden={creation==="ai"} className="manual-studio"><p className="creation-help">Pon un nombre, añade el enunciado y su respuesta. Las pistas son opcionales. Se guarda como borrador hasta que lo actives.</p><SkillEditor importOpen={creation==="pdf"}/></div>
  </section>;
}
