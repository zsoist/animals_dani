...
export function RefugeClient({cats,skills,queue,state,streak,userId}:{cats:never[];skills:Skill[];queue:Question[];state:ShelterState;streak:Streak;userId:string}){
  const [session,setSession]=useState<string|null>(null);
  const [playing,setPlaying]=useState(false);
  if(playing&&session)return <main className="shelter practice-mode"><Shelter cats={cats}/><Practice sessionId={session} queue={queue} skills={skills} state={state} streak={streak} userId={userId} onDone={()=>setPlaying(false)}/></main>;
