...
const stories:Record<CareReward,string>={food:'Milo ya se acercó a su plato. Cuenta contigo para preparar su comida de hoy.',bed:'Después de explorar tanto, a Milo le vendrá bien una camita cómoda. Hoy puedes preparársela.',box:'Milo encontró el rincón perfecto para una caja. Ayúdalo a estrenar su próximo escondite.',treat:'Hoy toca un pequeño premio para la familia. Una galletita para compartir después de tu reto.'};
export function MissionWelcome({today,completed,reward,busy,available,onStart}:{today:string;completed:boolean;reward:CareReward;busy:boolean;available:boolean;onStart:()=>void}) {
 ...
 <div className="welcome-art"><CatArt/>{!completed&&reward!=='treat'&&<RefugeProp kind={reward}/>}</div>
 <h2 id="welcome-title">{completed?'Hoy hiciste la diferencia':rewardCopy[reward].title}</h2>
 <p>{completed?'Tus gatos ya recibieron su cuidado de hoy. Puedes pasar a saludarlos o practicar un poquito más, a tu ritmo.':stories[reward]}</p>
