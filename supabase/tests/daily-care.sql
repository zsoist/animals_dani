...
begin
 ...
 for scenario in 1..3 loop
  update public.sessions set date=today-30 where user_id=uid and date=today;
  ...
  select food+beds+treats,boxes into food_before,boxes_before from public.shelter_state where user_id=uid;
  ...
  if (select food+boxes+beds+treats from public.shelter_state where user_id=uid)<>food_before+boxes_before+1 then raise exception 'Care duplicated';end if;
 end loop;
end $$;
rollback;
